...
## Questão 1) 
## Utilize as funções do pacote *lubridate* para executar o que se pede nas questões.

\vskip 1em

### a) Obtenha o dia da semana de cada corrida.

```{r q1a, include=FALSE}
# Uma forma 
uber2 <- mutate(uber2, dsemana = wday(`Data e hora`, label=T, abbr=F))
...
## Questão 2) 
## Utilize as funções dos pacotes *hms*, *lubridate* e *forcats* para executar as questões abaixo.
...
# Outra opção
uber2 %>% 
  mutate(hora_dia = hour(`Data e hora`)) %>% 
  count(hora_dia) %>% 
  print(n=24)
...
uber2 %>% 
  mutate(hora_dia = hour(`Data e hora`)) %>% 
  count(hora_dia) %>% 
  mutate(hora_dia = as.factor(hora_dia)) %>%
  ggplot() + 
  geom_col(aes(fct_reorder(hora_dia,n, .desc = T), n) ) + 
  xlab("Hora do dia") + 
